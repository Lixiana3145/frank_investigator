  ...
  class AuthorityLaunderingDetector
    ...
    def create_interaction(model, prompt, fingerprint)
      LlmInteraction.create!(
        investigation: @investigation,
        interaction_type: :authority_laundering,
        model_id: model,
        prompt_text: prompt,
      ...
      Rails.logger.warn("Failed to create authority laundering interaction: #{e.message}")
      nil
    end
    ...
    def complete_interaction(interaction, response, payload, elapsed_ms)
      ...
      Rails.logger.warn("Failed to update authority laundering interaction: #{e.message}")
    end
