  ...
  class SourceMisrepresentationDetector
    ...
    def create_interaction(model, prompt, fingerprint)
      LlmInteraction.create!(
        investigation: @investigation,
        interaction_type: :source_misrepresentation,
        model_id: model,
        prompt_text: prompt,
      ...
      Rails.logger.warn("Failed to create source misrepresentation interaction: #{e.message}")
      nil
    end
    ...
    def complete_interaction(interaction, response, payload, elapsed_ms)
      ...
      Rails.logger.warn("Failed to update source misrepresentation interaction: #{e.message}")
    end
